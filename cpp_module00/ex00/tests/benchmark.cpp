...
double benchmark_solution(const std::string& name, SolutionFn func, int ac, char** av) {
...
void run_chaos_scenario(const std::string& name, int num_args, int length_per_arg, int space_pct) {
    std::cout << "\n───────────────────────────────────────────\n";
    std::cout << " Scenario: " << name << "\n";
    std::cout << " Payload : " << num_args << " args x " << length_per_arg
              << " chars (~" << (num_args * length_per_arg) / 1000000.0 << " MB)\n";
    ...
    benchmark_solution("Other Solution (std::string)", other_solution, ac, &argv[0]);
    benchmark_solution("Optimized Streamer (In-Place)", optimized_solution, ac, &argv[0]);
