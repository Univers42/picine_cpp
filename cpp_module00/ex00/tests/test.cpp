...
private:
    ...
    // Helper to strip ANSI color codes (e.g., \033[33m) so tests compare pure text
...
public:
    MegaphoneTester(SolutionFn func) : target_func(func), passed(0), failed(0) {}

    // Highly scalable test runner
    void run_test(const std::string& label, const std::vector<std::string>& args, const std::string& expected) {
    ...
    void run_performance(const std::string& label, const std::string& pattern, int num_args, size_t expected_length) {
        std::cout << "\n── Performance: " << label << " ───────────\n";
        std::vector<char*> argv;
        char prog_name[] = "./megaphone";
        argv.push_back(prog_name);
        ...
        std::cout << (ok ? "[PASS]" : "[FAIL]") << " Processed in " << elapsed_ms << " ms.\n";
        if (!ok) {
...
std::vector<std::string> make_vec(const char* a=NULL, const char* b=NULL, const char* c=NULL) {
...
int main() {
    ...
    // ── boundary tests ──
    tester.run_test("boundary across args", make_vec("hello ", " world"), "HELLO WORLD");

    // ── noise fallback tests ──
    std::string noise = "* LOUD AND UNBEARABLE FEEDBACK NOISE *"; // Check your macro text!
    tester.run_test("empty string", make_vec(""), noise);
    tester.run_test("spaces only", make_vec("   ", "  "), noise);

    // ── performance test ──
    // 10,000 args of "   heLLo woRLd   " -> "HELLO WORLD" (11 chars * 10,000 = 110,000)
    // Plus 9,999 spaces separating the arguments = 119,999 pure characters.
    tester.run_performance("10k Heavy Whitespace", "   heLLo woRLd   ", 10000, 119999);
