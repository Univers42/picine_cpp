...
int main() {
  ...
  std::cout << C_BOLD << C_MAGENTA
            << "\n=== 2. EDGE CASE: HUMAN_B UNARMED ===" << C_RESET
            << std::endl;
  {
    HumanB paul("Pacifist Paul");

    std::cout << C_YELLOW
              << "-> Paul tries to attack without a weapon:" << C_RESET
              << std::endl;
    // If you didn't check for NULL in your attack() function, this line would
    // cause a Segfault!
    paul.attack();

    std::cout << C_YELLOW << "-> Paul finds a shiny longsword!" << C_RESET
              << std::endl;
            ...
            << "\n=== 3. EDGE CASE: EMPTY STRINGS ===" << C_RESET << std::endl;
  {
    ...
    std::cout << C_YELLOW
              << "-> Nameless HumanA with a nameless weapon:" << C_RESET
              << std::endl;
    ghost.attack();
  }
