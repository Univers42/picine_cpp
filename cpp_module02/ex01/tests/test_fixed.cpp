...
void test_subject_values() {
    ...
    if (std::abs(b.toFloat() - 10.0f) < 0.0001f &&
        ...
        std::cout << GREEN << "  [OK] Subject basic conversions" << RESET << std::endl;
    else
        std::cout << RED << "  [KO] Subject basic conversions" << RESET << std::endl;
}
...
void test_precision_limits() {
    ...
    if (std::abs(min_step.toFloat() - 0.00390625f) < 0.00000001f)
        std::cout << GREEN << "  [OK] Precision delta (1/256)" << RESET << std::endl;
    else
...
void test_negative_values() {
    ...
    if (neg_int.toInt() == -10 && neg_float.toInt() == -42)
        std::cout << GREEN << "  [OK] Negative value handling" << RESET << std::endl;
    else
...
int main(void) {
    TestMapping dispatchTable[] = {
        {"Subject Values", test_subject_values},
    ...
    std::cout << BOLD << "\n--- EX01 COMPREHENSIVE TEST SUITE ---" << RESET << std::endl;
