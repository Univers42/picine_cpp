...
/*   Updated: 2026/02/28 19:49:27 by dlesieur         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int bit_sub(int a, int b) {
    return bit_add(a, bit_add(~b, 1));
}

static long long ll_add(long long a, long long b) {
    while (b != 0) {
        long long carry = a & b;
        a = a ^ b;
        b = carry << 1;
...
static long long ll_sub(long long a, long long b) {
    return ll_add(a, ll_add(~b, 1));
}
...
bool Fixed::operator<=(const Fixed &other) const { return (*this < other) | (*this == other); }
bool Fixed::operator>=(const Fixed &other) const { return (*this > other) | (*this == other); }
...
Fixed Fixed::operator*(const Fixed &other) const {
    Fixed res;
    long long a = this->_fixedPointValue;
    long long b = other._fixedPointValue;
    long long sign_a = (a >> 63) & 1;
    long long sign_b = (b >> 63) & 1;
    long long final_sign = sign_a ^ sign_b;
...
Fixed Fixed::operator/(const Fixed &other) const {
    ...
    long long dividend = a << _fractionalBits;
    long long divisor = b;
    long long quotient = 0;

    for (int i = 39; i >= 0; i = bit_sub(i, 1)) {
        long long shifted_divisor = divisor << i;
        long long diff = ll_sub(dividend, shifted_divisor);
        if (((diff >> 63) & 1) == 0) {
            dividend = diff;
            quotient = ll_add(quotient, (long long)1 << i);
        }
    }

    if (final_sign) quotient = ll_sub(0, quotient);
    res.setRawBits((int)quotient);
    return res;
}
