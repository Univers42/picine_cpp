...
/*   Updated: 2026/02/28 19:41:00 by dlesieur         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void assert_test(bool condition, const std::string& testName, const std::string& details = "") {
    testsTotal++;
    if (condition) {
        testsPassed++;
    } else {
...
void test_arithmetic(int iterations) {
    std::cout << "Running " << iterations << " Arithmetic Fuzz Tests..." << std::endl;
    for (int i = 0; i < iterations; i++) {
        ...
        // Test Addition
        {
          ...
          add_err << "Fixed(" << f1 << " + " << f2 << ") = " << add_res.toFloat() << " | Expected: " << add_exp;
          assert_test(std::abs(add_res.toFloat() - add_exp) <= EPSILON, "Addition", add_err.str());
        }
        // Test Subtraction
        {
          Fixed sub_res = a - b;
          ...
          sub_err << "Fixed(" << f1 << " - " << f2 << ") = " << sub_res.toFloat() << " | Expected: " << sub_exp;
          assert_test(std::abs(sub_res.toFloat() - sub_exp) <= EPSILON, "Subtraction", sub_err.str());
        }
        // Test Multiplication
        {
          Fixed mul_res = a * b;
          ...
          mul_err << "Fixed(" << f1 << " * " << f2 << ") = " << mul_res.toFloat() << " | Expected: " << mul_exp;
          assert_test(std::abs(mul_res.toFloat() - mul_exp) <= EPSILON, "Multiplication", mul_err.str());
        }
        // Test Division
        {
          Fixed div_res = a / b;
          ...
          div_err << "Fixed(" << f1 << " / " << f2 << ") = " << div_res.toFloat() << " | Expected: " << div_exp;
          assert_test(std::abs(div_res.toFloat() - div_exp) <= EPSILON, "Division", div_err.str());
        }
    }
}

void test_comparisons(int iterations) {
    std::cout << "Running " << iterations << " Comparison Fuzz Tests..." << std::endl;
    for (int i = 0; i < iterations; i++) {
...
int main() {
    ...
    if (testsPassed == testsTotal) {
        std::cout << GREEN << "[SUCCESS] " << testsPassed << "/" << testsTotal << " Tests Passed!" << RESET << std::endl;
        std::cout << "The bitwise algorithms perfectly simulate FPU operations!" << std::endl;
    } else {
        std::cout << RED << "[FAILED] " << (testsTotal - testsPassed) << " tests failed." << RESET << std::endl;
    }
