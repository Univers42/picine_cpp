...
/*   Updated: 2026/02/28 21:54:47 by dlesieur         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// A standard 32-bit floating point cross product to act as the "Source of Truth"
float floatCrossProduct(float px, float py, float a_x, float a_y, float b_x, float b_y) {
    return (px - b_x) * (a_y - b_y) - (a_x - b_x) * (py - b_y);
}

// A standard 32-bit float BSP logic
bool standard_bsp(float ax, float ay, float bx, float by, float cx, float cy, float px, float py) {
    ...
    // because 8-bit Fixed math and 32-bit Float math handle tiny edge boundaries differently.
...
void run_fuzzer(int iterations) {
    std::cout << BOLD << "\n--- Running " << iterations << " BSP Fuzz Tests ---" << RESET << std::endl;
    ...
    // Extract the quantized (Fixed) values so the float algorithm sees exactly what the Fixed algorithm sees
    float ax = a.getX().toFloat(); float ay = a.getY().toFloat();
    float bx = b.getX().toFloat(); float by = b.getY().toFloat();
    float cx = c.getX().toFloat(); float cy = c.getY().toFloat();
    ...
    for (int i = 0; i < iterations; i++) {
        ...
        // Check if it's too close to the edge to be a fair test between 8-bit and 32-bit
        ...
        if (std::abs(d1) < 0.5f || std::abs(d2) < 0.5f || std::abs(d3) < 0.5f) {
            continue; // Skip boundary points to avoid precision-based false negatives
        }
        ...
        if (expected == actual) {
            ...
            std::cout << RED << "[FUZZ FAIL] " << RESET << "Point(" << px << ", " << py << ") "
                      << "| Expected: " << (expected ? "Inside" : "Outside")
                      << " | Actual: " << (actual ? "Inside" : "Outside") << std::endl;
        }
    }

    if (fuzz_passed == fuzz_total)
        std::cout << GREEN << "[SUCCESS] " << fuzz_passed << "/" << fuzz_total << " Fuzz Tests Passed!" << RESET << std::endl;
    else
        std::cout << RED << "[FAILED] " << (fuzz_total - fuzz_passed) << " Fuzz Tests Failed." << RESET << std::endl;
}
...
int main(void) {
    ...
    Point a2(-5.0f, -5.0f); Point b2(5.0f, -5.0f); Point c2(0.0f, 5.0f);
    ...
    if (testsPassed == testsTotal) {
        std::cout << GREEN << "All " << testsPassed << " tests perfectly simulate FPU BSP algorithms!" << RESET << std::endl;
    } else {
        std::cout << RED << "Only " << testsPassed << "/" << testsTotal << " tests passed." << RESET << std::endl;
    }
