...
BitcoinExchange &BitcoinExchange::operator=(const BitcoinExchange &other) {
  if (this != &other) _priceDb = other._priceDb;
  return *this;
}
...
void BitcoinExchange::loadDatabase(const std::string &dbPath) {
  ...
  for (size_t i = 0; i < doc.rowCount(); ++i) {
    ...
    if (endptr != NULL && endptr != price.c_str()) _priceDb[date] = val;
  }
}
...
void BitcoinExchange::processInputFile(const std::string &inputPath) const {
  ...
  while (std::getline(file, line)) {
    ...
    // Validate value
    double value = 0;
    if (!_isValidValue(valueStr, value)) continue;
...
RenderConfig BitcoinExchange::_getStyle(const std::string &name) {
  if (name == "matrix") return StylePresets::matrix();
  if (name == "fire") return StylePresets::fire();
  if (name == "corporate") return StylePresets::corporate();
  if (name == "newspaper") return StylePresets::newspaper();
  if (name == "elegant") return StylePresets::elegant();
  if (name == "minimal") return RenderConfig::minimal();
  // default: ocean
...
void BitcoinExchange::showDatabase(const std::string &styleName,
  ...
  Table tbl = db.table();
  if (maxRows > 0) tbl = Transform::limit(tbl, maxRows);
...
void BitcoinExchange::showStats() const {
  ...
  Statistics::ColumnStats stats =
      Statistics::analyze(db.table(), "Exchange Rate");
