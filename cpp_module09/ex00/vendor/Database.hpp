...
namespace Unicode {
    ...
    inline std::string pad(const std::string& str, size_t width, char align = 'l') {
...
public:
    ...
    std::string render(Table& table) {
        ...
        // Expand the last column if the footer text is wider than the total inner width.
        if (_config.showFooter && !_config.footerText.empty()) {
            ...
            // +2 accounts for the 1-char padding on each side inside the merged footer cell
