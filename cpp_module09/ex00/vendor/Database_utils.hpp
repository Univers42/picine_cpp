...
struct Query {
  // Predicate-based filtering (template to accept function pointers or
  // functors)
  template <typename Pred>
  static std::vector<Row> filter(const Table& table, Pred pred) {
  ...
  static std::vector<T> selectColumn(const Table& table,
  ...
  // Aggregate functions
  static double sum(const Table& table, const std::string& columnName) {
    std::vector<double> values =
        selectColumn<double>(table, columnName, &toDouble);
    return std::accumulate(values.begin(), values.end(), 0.0);
  }

  static double average(const Table& table, const std::string& columnName) {
    if (table.rowCount() == 0) return 0.0;
    return sum(table, columnName) / static_cast<double>(table.rowCount());
  }

  static double min(const Table& table, const std::string& columnName) {
    std::vector<double> values =
        selectColumn<double>(table, columnName, &toDouble);
  ...
  static double max(const Table& table, const std::string& columnName) {
    std::vector<double> values =
        selectColumn<double>(table, columnName, &toDouble);
    return values.empty() ? 0.0
  ...
  template <typename Pred>
  static size_t count(const Table& table, Pred pred) {
...
struct Transform {
  // Comparator functor (defined in struct scope to avoid local-type issues)
  ...
  static Table sortBy(const Table& table, const std::string& columnName,
                      bool ascending = true) {
    Table sorted;
    // copy columns
  ...
  static std::map<std::string, std::vector<Row> > groupBy(
      const Table& table, const std::string& columnName) {
  ...
  static Table selectColumns(const Table& table,
  ...
  static Table limit(const Table& table, size_t maxRows) {
    Table result;
    const std::vector<Column>& tcols = table.columns();
    for (size_t i = 0; i < tcols.size(); ++i) {
...
struct Generator {
  ...
  static Database createSampleEmployees(size_t count = 10) {
    Database db;
  ...
  static Database createSampleSales(size_t count = 8) {
    Database db;
    ...
    const char* monthsArr[] = {"Jan", "Feb", "Mar", "Apr",
                               "May", "Jun", "Jul", "Aug"};
...
struct StylePresets {
  ...
  static RenderConfig elegant() { return RenderConfig::elegant(); }
};  // struct StylePresets
...
struct Export {
  // Export to CSV
  static void toCsv(const Table& table, const std::string& path) {
    std::ofstream file;
  ...
  // Export to HTML
  static std::string toHtml(Table& table,
                            const std::string& title = "Data Table") {
  ...
  // Export to Markdown
  static std::string toMarkdown(Table& table) {
    ...
    for (size_t r = 0; r < rows.size(); ++r) {
      for (size_t c = 0; c < cols.size(); ++c) {
        ...
        md << "| "
           << Unicode::pad(value, cols[c].width(), cols[c].getAlignChar())
           << " ";
      }
      md << "|\n";
    }
...
struct Statistics {
  ...
  static ColumnStats analyze(const Table& table,
                             const std::string& columnName) {
    ColumnStats stats;
    ...
    std::vector<double> values =
        Query::selectColumn<double>(table, columnName, &Query::toDouble);
  ...
  static Database createStatsTable(const ColumnStats& stats,
                                   const std::string& columnName) {
    Database db;
