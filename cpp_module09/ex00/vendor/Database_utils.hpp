...
namespace Query {
    ...
    // Aggregate functions
    inline double sum(const Table& table, const std::string& columnName) {
        std::vector<double> values = selectColumn<double>(table, columnName, &toDouble);
        return std::accumulate(values.begin(), values.end(), 0.0);
    }

    inline double average(const Table& table, const std::string& columnName) {
        if (table.rowCount() == 0) return 0.0;
        return sum(table, columnName) / static_cast<double>(table.rowCount());
    }

    inline double min(const Table& table, const std::string& columnName) {
        std::vector<double> values = selectColumn<double>(table, columnName, &toDouble);
        return values.empty() ? 0.0 : *std::min_element(values.begin(), values.end());
    }

    inline double max(const Table& table, const std::string& columnName) {
        std::vector<double> values = selectColumn<double>(table, columnName, &toDouble);
...
namespace Transform {
    ...
    inline Table sortBy(const Table& table, const std::string& columnName, bool ascending = true) {
        Table sorted;
        // copy columns
    ...
    inline std::map<std::string, std::vector<Row> > groupBy(const Table& table, const std::string& columnName) {
    ...
    inline Table selectColumns(const Table& table, const std::vector<std::string>& columnNames) {
        Table result;
    ...
    inline Table limit(const Table& table, size_t maxRows) {
        Table result;
        const std::vector<Column>& tcols = table.columns();
        for (size_t i = 0; i < tcols.size(); ++i) {
...
namespace Generator {
    ...
    inline Database createSampleEmployees(size_t count = 10) {
        Database db;
    ...
    inline Database createSampleSales(size_t count = 8) {
        Database db;
...
namespace Export {
    // Export to CSV
    inline void toCsv(const Table& table, const std::string& path) {
        std::ofstream file;
    ...
    // Export to HTML
    inline std::string toHtml(Table& table, const std::string& title = "Data Table") {
    ...
    // Export to Markdown
    inline std::string toMarkdown(Table& table) {
...
namespace Statistics {
    ...
    inline ColumnStats analyze(const Table& table, const std::string& columnName) {
        ColumnStats stats;
    ...
    inline Database createStatsTable(const ColumnStats& stats, const std::string& columnName) {
        Database db;
