...
class Date {
 public:
  ...
  Date subtractDays(int days) const { return addDays(-days); }
  ...
  Date subtractMonths(int months) const { return addMonths(-months); }
  ...
  Date subtractYears(int years) const { return addYears(-years); }
  ...
  Date operator-(int days) const { return subtractDays(days); }

  int operator-(const Date& other) const { return daysBetween(other); }
  ...
  std::string toString(const std::string& format = "YYYY-MM-DD") const {
    if (format == "YYYY-MM-DD" || format == "ISO") {
      ...
      oss << monthName() << " " << _day << ", " << _year;
      return oss.str();
    } else if (format == "DD Month YYYY") {
      std::ostringstream oss;
      oss << _day << " " << monthName() << " " << _year;
      return oss.str();
    } else if (format == "Weekday, Month DD, YYYY") {
      std::ostringstream oss;
      oss << weekdayName() << ", " << monthName() << " " << _day << ", "
          << _year;
      return oss.str();
    }
    return toISO();
  ...
  std::string toISO() const {
    std::ostringstream oss;
    oss << std::setfill('0') << std::setw(4) << _year << "-" << std::setw(2)
        << _month << "-" << std::setw(2) << _day;
    return oss.str();
  }
 ...
 private:
  ...
  int toJulianDay() const {
    ...
    return _day + (153 * m + 2) / 5 + 365 * y + y / 4 - y / 100 + y / 400 -
           32045;
  }
  ...
  std::string toDMY() const {
    std::ostringstream oss;
    oss << std::setfill('0') << std::setw(2) << _day << "/" << std::setw(2)
        << _month << "/" << std::setw(4) << _year;
    return oss.str();
  }

  std::string toMDY() const {
    std::ostringstream oss;
    oss << std::setfill('0') << std::setw(2) << _month << "/" << std::setw(2)
        << _day << "/" << std::setw(4) << _year;
    return oss.str();
  }

  std::string toYMD(char sep) const {
    std::ostringstream oss;
    oss << std::setfill('0') << std::setw(4) << _year << sep << std::setw(2)
        << _month << sep << std::setw(2) << _day;
    return oss.str();
  }
};
...
class DateRange {
 public:
  ...
  std::vector<Date> toVector() const {
    ...
    for (DateRange::Iterator it = begin(); it != end(); ++it)
      dates.push_back(*it);
    return dates;
  }
...
struct DateUtils {
  ...
  static Date firstOfMonth(int year, int month) { return Date(year, month, 1); }

  // Get last day of month
  static Date lastOfMonth(int year, int month) {
    int days = Date::daysInMonth(year, month);
    return Date(year, month, days);
  }

  // Get first day of year
  static Date firstOfYear(int year) { return Date(year, 1, 1); }

  // Get last day of year
  static Date lastOfYear(int year) { return Date(year, 12, 31); }

  // Get next weekday occurrence
  static Date nextWeekday(const Date& from, int targetWeekday) {
  ...
  static Date previousWeekday(const Date& from, int targetWeekday) {
    int currentWeekday = from.weekday();
  ...
  static std::vector<Date> datesInMonth(int year, int month) {
  ...
  static std::vector<Date> datesInYear(int year) {
  ...
  static bool isWeekday(const Date& date) { return !isWeekend(date); }

  // Count business days between two dates (Monday-Friday)
  static int businessDaysBetween(const Date& start, const Date& end) {
  ...
  static Date addBusinessDays(const Date& from, int days) {
    Date result = from;
    int added = 0;
  ...
  static int age(const Date& birthDate,
                 const Date& currentDate = Date::today()) {
  ...
  // Calculate quarters
  static int quarter(const Date& date) { return (date.month() - 1) / 3 + 1; }

  static Date quarterStart(int year, int quarter) {
    int month = (quarter - 1) * 3 + 1;
    return Date(year, month, 1);
  }

  static Date quarterEnd(int year, int quarter) {
    int month = quarter * 3;
