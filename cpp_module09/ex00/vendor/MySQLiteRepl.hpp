...
/*   Updated: 2026/03/01 14:39:51 by dlesieur         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//    LOAD DIR '<directory>'                    — import all CSVs from a folder
...
struct Token {
  ...
  Token(TK::Type t, const std::string& v, size_t p)
...
class Lexer {
 public:
  ...
  std::vector<Token> tokenize() {
    ...
    while (_pos < _input.size()) {
      ...
      // Single-char symbols
      if (c == '*') {
        tokens.push_back(Token(TK::STAR, "*", start));
        ++_pos;
        continue;
      }
      if (c == ',') {
        tokens.push_back(Token(TK::COMMA, ",", start));
        ++_pos;
        continue;
      }
      if (c == '(') {
        tokens.push_back(Token(TK::LPAREN, "(", start));
        ++_pos;
        continue;
      }
      if (c == ')') {
        tokens.push_back(Token(TK::RPAREN, ")", start));
        ++_pos;
        continue;
      }
      if (c == '.') {
        tokens.push_back(Token(TK::DOT, ".", start));
        ++_pos;
        continue;
      }
      if (c == ';') {
        tokens.push_back(Token(TK::SEMICOLON, ";", start));
        ++_pos;
        continue;
      }
      ...
      // Two-char operators
      if (c == '!' && _peek(1) == '=') {
        tokens.push_back(Token(TK::NEQ, "!=", start));
        _pos += 2;
        continue;
      }
      if (c == '<' && _peek(1) == '=') {
        tokens.push_back(Token(TK::LTE, "<=", start));
        _pos += 2;
        continue;
      }
      if (c == '>' && _peek(1) == '=') {
        tokens.push_back(Token(TK::GTE, ">=", start));
        _pos += 2;
        continue;
      }
      if (c == '<' && _peek(1) == '>') {
      ...
      if (c == '<') {
        tokens.push_back(Token(TK::LT, "<", start));
        ++_pos;
        continue;
      }
      if (c == '>') {
        tokens.push_back(Token(TK::GT, ">", start));
        ++_pos;
        continue;
      }
      if (c == '=') {
        tokens.push_back(Token(TK::EQ, "=", start));
        ++_pos;
        continue;
      }
      ...
      if (std::isdigit(c) || (c == '-' && _pos + 1 < _input.size() &&
                              std::isdigit(_input[_pos + 1]))) {
 ...
 private:
  ...
  Token _readNumberOrDate() {
    ...
    while (_pos < _input.size() &&
           (std::isdigit(_input[_pos]) || _input[_pos] == '.')) {
      val += _input[_pos];
      ++_pos;
    }
  ...
  Token _readIdentifier() {
    ...
    if (it != _keywords.end()) return Token(it->second, val, start);
  ...
  struct Statement {
    ...
    Statement()
        : type(STMT_UNKNOWN),
          orderAsc(true),
          limitN(0),
          alterAction(ALT_ADD_COL),
          alterColType(ColumnType::STRING),
          aggFunc(AGG_COUNT),
          exportFmt(EXP_CSV) {}
  };
};  // struct AST
  ...
  AST::Statement parse() {
    ...
    switch (_cur().type) {
      ...
      case TK::SHOW:
        return _parseShow();
      case TK::DESCRIBE:
        return _parseDescribe();
      case TK::COUNT_KW:
        return _parseAggregate(AST::AGG_COUNT);
      case TK::SUM_KW:
        return _parseAggregate(AST::AGG_SUM);
      case TK::AVG_KW:
        return _parseAggregate(AST::AGG_AVG);
      case TK::MIN_KW:
        return _parseAggregate(AST::AGG_MIN);
      case TK::MAX_KW:
        return _parseAggregate(AST::AGG_MAX);
      case TK::STATS_KW:
        return _parseStats();
      case TK::EXPORT:
        return _parseExport();
      ...
      case TK::HELP: {
        AST::Statement s;
        s.type = AST::STMT_HELP;
        return s;
      }
      case TK::QUIT:
      case TK::EXIT: {
        AST::Statement s;
        s.type = AST::STMT_QUIT;
        return s;
      }
      default:
 ...
 private:
  ...
  std::string _readName() {
    if (_cur().type == TK::IDENTIFIER ||
        (_cur().type >= TK::SELECT && _cur().type <= TK::T_BOOLEAN)) {
      return _advance().value;
    }
    throw std::runtime_error(std::string("Expected name, got '") +
                             _cur().value + "'");
  }
  ...
  std::string _readValue() {
    if (_cur().type == TK::STRING_LIT || _cur().type == TK::NUMBER_LIT)
      return _advance().value;
    if (_cur().type == TK::IDENTIFIER) return _advance().value;
    // Also allow keyword tokens as values (e.g. "true", "false", type names
    // used as values)
    if (_cur().type >= TK::SELECT && _cur().type <= TK::T_BOOLEAN)
      return _advance().value;
    throw std::runtime_error(std::string("Expected value, got '") +
                             _cur().value + "'");
  }
  ...
  ColumnType::Type _readType() {
    switch (_cur().type) {
      ...
      default:
        return ColumnType::STRING;  // implicit STRING if omitted
    }
  }
  ...
  std::vector<AST::Condition> _parseWhere() {
    ...
    while (true) {
      ...
      if (_match(TK::EQ))
        c.op = "=";
      else if (_match(TK::NEQ))
        c.op = "!=";
      else if (_match(TK::LT))
        c.op = "<";
      else if (_match(TK::GT))
        c.op = ">";
      else if (_match(TK::LTE))
        c.op = "<=";
      else if (_match(TK::GTE))
        c.op = ">=";
      else
        throw std::runtime_error(
            "Expected comparison operator after column name in WHERE");
  ...
  AST::Statement _parseLoad() {
    ...
    // LOAD DIR '<directory>' — load all CSVs from a folder
    if (_cur().type == TK::DIR) {
      _advance();  // DIR
      s.type = AST::STMT_LOAD_DIR;
      if (_cur().type != TK::STRING_LIT)
        throw std::runtime_error(
            "LOAD DIR expects a directory path in quotes, "
            "e.g. LOAD DIR 'samples/'");
      s.loadPath = _advance().value;
      return s;
    }

    // LOAD '<file>' [AS name] — load single CSV
    s.type = AST::STMT_LOAD;
    if (_cur().type != TK::STRING_LIT)
      throw std::runtime_error(
          "LOAD expects a file path in quotes, e.g. LOAD 'file.csv' AS name");
    s.loadPath = _advance().value;
    if (_match(TK::AS))
  ...
  AST::Statement _parseShow() {
    ...
    if (_match(TK::TABLES)) {
      ...
      throw std::runtime_error(
          "SHOW expects TABLES, DATABASES, or TABLE <name>");
    }
    return s;
  }
  ...
  AST::Statement _parseExport() {
    ...
    if (_match(TK::CSV_KW))
      s.exportFmt = AST::EXP_CSV;
    else if (_match(TK::HTML_KW))
      s.exportFmt = AST::EXP_HTML;
    else if (_match(TK::MARKDOWN_KW))
      s.exportFmt = AST::EXP_MARKDOWN;
    else
      throw std::runtime_error(
          "EXPORT: expected CSV, HTML, or MARKDOWN after TO");

    if (s.exportFmt != AST::EXP_MARKDOWN) {
      if (_cur().type != TK::STRING_LIT)
        throw std::runtime_error(
            "EXPORT to CSV/HTML requires a path in quotes");
...
class Executor {
 public:
  ...
  std::string execute(const AST::Statement& stmt) {
    switch (stmt.type) {
      case AST::STMT_SELECT:
        return _execSelect(stmt);
      case AST::STMT_INSERT:
        return _execInsert(stmt);
      case AST::STMT_UPDATE:
        return _execUpdate(stmt);
      case AST::STMT_DELETE:
        return _execDelete(stmt);
      case AST::STMT_CREATE:
        return _execCreate(stmt);
      case AST::STMT_DROP_TABLE:
        return _execDrop(stmt);
      case AST::STMT_ALTER:
        return _execAlter(stmt);
      case AST::STMT_LOAD:
        return _execLoad(stmt);
      case AST::STMT_LOAD_DIR:
        return _execLoadDir(stmt);
      case AST::STMT_TABLES:
        return _execTables();
      case AST::STMT_SHOW_DATABASES:
        return _execShowDatabases();
      case AST::STMT_DESCRIBE:
        return _execDescribe(stmt);
      case AST::STMT_AGGREGATE:
        return _execAggregate(stmt);
      case AST::STMT_STATS:
        return _execStats(stmt);
      case AST::STMT_EXPORT:
        return _execExport(stmt);
      case AST::STMT_STYLE:
        _styleName = _toLower(stmt.styleName);
        return _info("Style set to '" + _styleName + "'.");
      case AST::STMT_HELP:
        return _helpText();
      case AST::STMT_QUIT:
        return "";
      default:
        return _err("Unknown statement.");
    }
  }
 ...
 private:
  ...
  Database& _getTable(const std::string& name) {
    ...
    if (it == _catalog.end())
      throw std::runtime_error(
          ...
          "' not found. Use TABLES to list loaded tables.");
    return it->second;
  }
  ...
  //  Given a user-typed column name (any case), find the actual column
  //  name stored in the table.  Returns the canonical name, or the
  //  original input if nothing matched (so downstream errors stay clear).

  static std::string _resolveColumn(const Table& tbl,
                                    const std::string& input) {
    const std::vector<Column>& cols = tbl.columns();
    // Exact match first
    for (size_t i = 0; i < cols.size(); ++i)
      if (cols[i].name() == input) return input;
    // Case-insensitive fallback
    std::string low = input;
    for (size_t i = 0; i < low.size(); ++i)
      low[i] =
          static_cast<char>(std::tolower(static_cast<unsigned char>(low[i])));
    for (size_t i = 0; i < cols.size(); ++i) {
      std::string cn = cols[i].name();
      std::string cnLow = cn;
      for (size_t j = 0; j < cnLow.size(); ++j)
        cnLow[j] = static_cast<char>(
            std::tolower(static_cast<unsigned char>(cnLow[j])));
      if (cnLow == low) return cn;
    }
    return input;  // not found — return as-is
  }

  // Resolve all condition column names against a table
  static void _resolveConditions(const Table& tbl,
                                 std::vector<AST::Condition>& conds) {
    for (size_t i = 0; i < conds.size(); ++i)
      conds[i].column = _resolveColumn(tbl, conds[i].column);
  }
  ...
  static bool _matchRow(const Row& row,
                        const std::vector<AST::Condition>& conds) {
  ...
  Table _buildResult(const Table& source, const std::vector<std::string>& cols,
    ...
    if (cols.empty()) {
      // all columns
      for (size_t i = 0; i < srcCols.size(); ++i) result.addColumn(srcCols[i]);
    } else {
      for (size_t i = 0; i < cols.size(); ++i) {
        bool found = false;
        ...
        if (!found) result.addColumn(Column(cols[i]));
      }
    }

    for (size_t i = 0; i < rows.size(); ++i) result.addRow(rows[i]);
  ...
  std::string _execSelect(const AST::Statement& s) {
    ...
    std::vector<AST::Condition> conds = s.conditions;
    _resolveConditions(tbl, conds);

    std::vector<std::string> selCols = s.columns;
    for (size_t i = 0; i < selCols.size(); ++i)
      selCols[i] = _resolveColumn(tbl, selCols[i]);

    // Filter rows
    std::vector<Row> rows;
    const std::vector<Row>& allRows = tbl.rows();
    for (size_t i = 0; i < allRows.size(); ++i) {
      if (_matchRow(allRows[i], conds)) rows.push_back(allRows[i]);
    }

    // Sort
    if (!s.orderColumn.empty()) {
      std::string resolvedOrder = _resolveColumn(tbl, s.orderColumn);
      std::sort(rows.begin(), rows.end(),
                Transform::RowComparator(resolvedOrder, s.orderAsc));
    }

    // Limit
    if (s.limitN > 0 && rows.size() > s.limitN) rows.resize(s.limitN);

    Table result = _buildResult(tbl, selCols, rows);

    std::ostringstream footer;
    footer << rows.size() << " row" << (rows.size() != 1 ? "s" : "")
           << " selected";
  ...
  std::string _execInsert(const AST::Statement& s) {
    ...
    for (size_t i = 0; i < s.insertCols.size(); ++i)
      rowData[_resolveColumn(tbl, s.insertCols[i])] = s.insertVals[i];
  ...
  std::string _execUpdate(const AST::Statement& s) {
    ...
    // Resolve SET clause column names
    std::map<std::string, std::string> resolvedSet;
    for (std::map<std::string, std::string>::const_iterator it =
             s.setClauses.begin();
         it != s.setClauses.end(); ++it) {
      resolvedSet[_resolveColumn(tbl, it->first)] = it->second;
    }

    size_t affected = 0;

    for (size_t i = 0; i < rows.size(); ++i) {
      if (_matchRow(rows[i], conds)) {
        for (std::map<std::string, std::string>::const_iterator it =
                 resolvedSet.begin();
             it != resolvedSet.end(); ++it) {
  ...
  std::string _execDelete(const AST::Statement& s) {
    ...
    std::vector<Row> kept;
    for (size_t i = 0; i < rows.size(); ++i) {
      if (!_matchRow(rows[i], conds)) kept.push_back(rows[i]);
    }
    rows = kept;
  ...
  std::string _execCreate(const AST::Statement& s) {
    ...
    std::ostringstream oss;
    oss << "Table '" << s.tableName << "' created with " << s.createCols.size()
        << " column(s).";
    return _info(oss.str());
  }
  ...
  std::string _execAlter(const AST::Statement& s) {
    ...
    switch (s.alterAction) {
      ...
      case AST::ALT_DROP_COL: {
        bool found = false;
        for (std::vector<Column>::iterator it = cols.begin(); it != cols.end();
             ++it) {
        ...
        return _info("Column '" + s.alterCol + "' dropped from '" +
                     s.tableName + "'.");
      }
      case AST::ALT_RENAME_COL: {
        for (size_t i = 0; i < cols.size(); ++i) {
          if (cols[i].name() == s.alterCol) {
            cols[i] =
                Column(s.alterNewName, cols[i].type(), cols[i].alignment());
            ...
            return _info("Column '" + s.alterCol + "' renamed to '" +
                         s.alterNewName + "'.");
          }
        }
      ...
      case AST::ALT_MODIFY_COL: {
        for (size_t i = 0; i < cols.size(); ++i) {
          if (cols[i].name() == s.alterCol) {
            ...
            return _info("Column '" + s.alterCol + "' type changed to " +
                         _typeToStr(s.alterColType) + ".");
          }
        }
        return _err("Column '" + s.alterCol + "' not found.");
      }
  ...
  std::string _execLoad(const AST::Statement& s) {
    ...
    oss << "Loaded '" << s.loadPath << "' as '" << s.loadAlias << "' ("
        << db.count() << " rows, " << db.table().columnCount() << " columns).";
    return _info(oss.str());
  }

  // ── LOAD DIR ────────────────────────────────────────────────────────

  std::string _execLoadDir(const AST::Statement& s) {
    DIR* d = opendir(s.loadPath.c_str());
    if (!d)
      throw std::runtime_error(std::string("Cannot open directory: '") +
                               s.loadPath + "'");
    ...
    while ((entry = readdir(d)) != NULL) {
      ...
      if (ext == ".csv") files.push_back(name);
    }
    closedir(d);
    std::sort(files.begin(), files.end());

    for (size_t f = 0; f < files.size(); ++f) {
      std::string fullPath = s.loadPath;
      if (!fullPath.empty() && fullPath[fullPath.size() - 1] != '/')
        fullPath += '/';
      fullPath += files[f];

      std::string tableName = files[f].substr(0, files[f].size() - 4);

      Database db;
      RenderConfig cfg;
      cfg.autoIncrementId = true;
      try {
        db.loadFromCsv(fullPath, true, cfg);
        _catalog[tableName] = db;
        oss << "  \033[32m+\033[0m " << fullPath
            << "  \033[90m\xe2\x86\x92\033[0m '" << tableName << "' ("
            << db.count() << " rows)\n";
        ++loaded;
      } catch (const std::exception& e) {
        oss << "  \033[31m\xe2\x9c\x97\033[0m " << fullPath << ": " << e.what()
            << "\n";
      }
    }

    if (loaded == 0)
      return _info("No CSV files found in '" + s.loadPath + "'.");

    std::ostringstream result;
    result << "\n" << oss.str() << "\n";
    std::ostringstream summary;
    summary << loaded << " CSV file" << (loaded != 1 ? "s" : "")
            << " loaded from '" << s.loadPath << "'.";
    result << _info(summary.str());
    return result.str();
  }
  ...
  std::string _execTables() {
    ...
    Database listing;
    listing.addColumn("Tables_in_mysqlite", ColumnType::STRING,
                      Alignment::LEFT);
    ...
    std::ostringstream footer;
    footer << _catalog.size() << " row" << (_catalog.size() != 1 ? "s" : "")
           << " in set";
  ...
  std::string _execDescribe(const AST::Statement& s) {
    ...
    for (size_t i = 0; i < cols.size(); ++i) {
      ...
      row["Alignment"] = (cols[i].alignment() == Alignment::LEFT)    ? "LEFT"
                         : (cols[i].alignment() == Alignment::RIGHT) ? "RIGHT"
                                                                     : "CENTER";
      // Count non-empty
      size_t nonEmpty = 0;
      std::string sample;
    ...
    std::ostringstream footer;
    footer << "'" << s.tableName << "': " << cols.size() << " columns, "
           << tbl.rowCount() << " rows";
  ...
  std::string _execAggregate(const AST::Statement& s) {
    ...
    const std::vector<Row>& allRows = tbl.rows();
    for (size_t i = 0; i < allRows.size(); ++i) {
      if (_matchRow(allRows[i], s.conditions)) rows.push_back(allRows[i]);
    }
    ...
    for (size_t i = 0; i < rows.size(); ++i) filtered.addRow(rows[i]);
  ...
  std::string _execStats(const AST::Statement& s) {
    Database& db = _getTable(s.tableName);
    Statistics::ColumnStats stats =
        Statistics::analyze(db.table(), s.aggColumn);
    ...
    cfg.showFooter = true;
    cfg.footerText =
        "Statistics for '" + s.aggColumn + "' in '" + s.tableName + "'";
  ...
  std::string _execExport(const AST::Statement& s) {
    ...
    switch (s.exportFmt) {
      case AST::EXP_CSV:
        ...
        return _info("Table '" + s.tableName +
                     "' exported to CSV: " + s.exportPath);
      case AST::EXP_HTML: {
  ...
  std::string _helpText() const {
    ...
    h << "\033[1;96m"
      ...
      << "    LOAD \033[36m'file.csv'\033[0m AS \033[33mname\033[0m       "
         "Import a CSV file\n"
      << "    LOAD DIR \033[36m'path/'\033[0m               "
      ...
      << "    SHOW TABLE \033[33mname\033[0m                (alias for "
         "DESCRIBE)\n"
      << "    DESCRIBE \033[33mtable\033[0m                 Show table schema\n"
      << "    DROP TABLE \033[33mtable\033[0m               Remove table from "
         "catalog\n"
      << "\n"
      << "\033[1;93m  Queries\033[0m\n"
      << "    SELECT \033[36m*|col,...\033[0m FROM \033[33mtable\033[0m    "
         "Query rows\n"
      << "      [WHERE \033[36mcol\033[0m \033[35mop\033[0m \033[36mval\033[0m "
         "[AND|OR ...]]  Filter\n"
      << "      [ORDER BY \033[36mcol\033[0m [ASC|DESC]]   Sort\n"
      << "      [LIMIT \033[36mn\033[0m]                   Limit rows\n"
      << "\n"
      << "\033[1;93m  Data Manipulation\033[0m\n"
      << "    INSERT INTO \033[33mtable\033[0m (\033[36mcols\033[0m) VALUES "
         "(\033[36mvals\033[0m)\n"
      << "    UPDATE \033[33mtable\033[0m SET "
         "\033[36mcol\033[0m=\033[36mval\033[0m [WHERE ...]\n"
      ...
      << "    ALTER \033[33mtable\033[0m ADD COLUMN \033[36mname\033[0m "
         "[\033[35mtype\033[0m]\n"
      << "    ALTER \033[33mtable\033[0m DROP COLUMN \033[36mname\033[0m\n"
      << "    ALTER \033[33mtable\033[0m RENAME COLUMN \033[36mold\033[0m TO "
         "\033[36mnew\033[0m\n"
      << "    ALTER \033[33mtable\033[0m MODIFY COLUMN \033[36mcol\033[0m "
         "\033[35mtype\033[0m\n"
      ...
      << "    SUM|AVG|MIN|MAX \033[36mcol\033[0m FROM \033[33mtable\033[0m "
         "[WHERE ...]\n"
      << "    STATS \033[36mcol\033[0m FROM \033[33mtable\033[0m          Full "
         "statistics\n"
      << "\n"
      ...
      << "    STYLE \033[36mname\033[0m                    "
         "ocean|matrix|fire|corporate|\n"
      ...
      << "    QUIT | EXIT                    Leave the REPL\n"
      << "\n"
      << "\033[90m  Operators: =  !=  <  >  <=  >=  |  Strings: 'quoted'  |  "
         ";optional\033[0m\n";
    return h.str();
  }
};
...
class Repl {
 public:
  ...
  ~Repl() { write_history(_historyFile.c_str()); }
  ...
  void run() {
    ...
    sa.sa_handler = _sigintHandler;
    sa.sa_flags = 0;
    sigemptyset(&sa.sa_mask);
    sigaction(SIGINT, &sa, &oldSa);

    static const char* PROMPT_MAIN =
        "\001\033[1;96m\002mysqlite\001\033[0m\033[96m\002>\001\033[0m\002 ";
    ...
    while (true) {
      ...
      if (trimmed[trimmed.size() - 1] != ';') continue;
 ...
 private:
  ...
  void _printBanner() const {
    std::cout << "\033[1;96m"
              ...
              << " ╠═══════════════════════════════════════════════════════╣\n"
              << " ║  Type \033[93mHELP\033[1;96m for commands  •  "
                 "\033[93mQUIT;\033[1;96m to exit            ║\n"
