...
struct Colors {
  ...
  class RGB {
   public:
    ...
    std::string toHex() const {
      ...
      oss << "#" << std::hex << std::setfill('0') << std::setw(2) << (int)r
    ...
    RGB lighten(double factor) const {
      ...
      return RGB(std::min(255, (int)(r + (255 - r) * factor)),
    ...
    RGB darken(double factor) const {
      factor = std::max(0.0, std::min(1.0, factor));
      return RGB(std::max(0, (int)(r * (1.0 - factor))),
    ...
    RGB desaturate(double factor) const { return saturate(-factor); }

    // Mix two colors
    static RGB mix(const RGB& c1, const RGB& c2, double ratio = 0.5) {
      ratio = std::max(0.0, std::min(1.0, ratio));
      return RGB((unsigned char)(c1.r * (1 - ratio) + c2.r * ratio),
                 (unsigned char)(c1.g * (1 - ratio) + c2.g * ratio),
                 (unsigned char)(c1.b * (1 - ratio) + c2.b * ratio));
    }

    // Complementary color
    RGB complement() const { return RGB(255 - r, 255 - g, 255 - b); }
  ...
  class ColorScheme {
   public:
    ...
    static RGB complementary(const RGB& base) { return base.complement(); }
  ...
  class Formatter {
   public:
    ...
    std::string colorize(const std::string& text, const RGB& fg,
                         const RGB& bg) const {
      std::ostringstream oss;
      oss << getEscapeCode(fg, false) << getEscapeCode(bg, true) << text
          << "\033[0m";
      return oss.str();
    }
   ...
   private:
    std::string colorize(const std::string& text, const RGB& color,
    ...
    std::string getEscapeCode(const RGB& color, bool background) const {
      ...
      switch (mode) {
        ...
        case ANSI_256:
          ...
          break;
        case TRUE_COLOR:
          oss << (background ? "48;2;" : "38;2;") << (int)color.r << ";"
  ...
  static RGB parse(const std::string& str) {
  ...
  static double distance(const RGB& c1, const RGB& c2) {
  ...
  static std::string closestName(const RGB& color) {
    double minDist = 1e9;
    std::string closest = "unknown";
    std::vector<std::pair<std::string, RGB> > namedColors;
    namedColors.push_back(
        std::make_pair(std::string("black"), Palette::Black()));
    namedColors.push_back(
        std::make_pair(std::string("white"), Palette::White()));
    namedColors.push_back(std::make_pair(std::string("red"), Palette::Red()));
    namedColors.push_back(
        std::make_pair(std::string("green"), Palette::Green()));
    namedColors.push_back(std::make_pair(std::string("blue"), Palette::Blue()));
    namedColors.push_back(
        std::make_pair(std::string("yellow"), Palette::Yellow()));
    namedColors.push_back(
        std::make_pair(std::string("magenta"), Palette::Magenta()));
    ...
    namedColors.push_back(
        std::make_pair(std::string("orange"), Palette::Orange()));
    namedColors.push_back(
        std::make_pair(std::string("purple"), Palette::Purple()));
