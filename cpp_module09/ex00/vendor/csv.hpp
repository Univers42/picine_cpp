...
struct RowComparator {
	...
	bool operator()(const Row& a, const Row& b) const {
		const std::string va = (a.size() > static_cast<size_t>(idx)) ? a[idx] : std::string();
		const std::string vb = (b.size() > static_cast<size_t>(idx)) ? b[idx] : std::string();
...
namespace Aggregate {
    ...
    inline size_t countValues(const Document& doc, const std::string& column) {
    ...
    inline double sum(const Document& doc, const std::string& column) {
        int idx = doc.getColumnIndex(column);
        if (idx < 0) return 0.0;
        double total = 0.0;
    ...
    inline double average(const Document& doc, const std::string& column) {
        size_t cnt = countValues(doc, column);
        if (cnt == 0) return 0.0;
        return sum(doc, column) / static_cast<double>(cnt);
    }

    inline double min(const Document& doc, const std::string& column) {
        int idx = doc.getColumnIndex(column);
        if (idx < 0) return 0.0;
        bool found = false;
        double m = 0.0;
    ...
    inline double max(const Document& doc, const std::string& column) {
        int idx = doc.getColumnIndex(column);
        if (idx < 0) return 0.0;
        bool found = false;
        double M = 0.0;
    ...
    inline double stddev(const Document& doc, const std::string& column) {
        size_t cnt = countValues(doc, column);
        if (cnt == 0) return 0.0;
