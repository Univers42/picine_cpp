...
static void ensure_locale_initialized() {
  ...
  if (inited) return;
  inited = true;
  /* Try environment / common UTF-8 locales; fall back silently if none
   * available. */
...
// testing::internal::String member function definitions

const char *testing::internal::String::CloneCString(const char *c_str) {
...
bool testing::internal::String::CStringEquals(const char *lhs,
                                              const char *rhs) {
  if (lhs == rhs) return true;  // covers both nullptr
...
std::string testing::internal::String::ShowWideCString(
  ...
  if (!wide_c_str) return std::string("(null)");
#if defined(__cpp_lib_codecvt) || defined(_MSC_VER)
  try {
...
std::string testing::internal::String::FormatIntWidth2(int val) {
  std::ostringstream ss;
  ss << std::setw(2) << std::setfill('0') << val;
  return ss.str();
}

std::string testing::internal::String::FormatIntWidthN(int val, int width) {
  std::ostringstream ss;
  if (width < 0) width = 0;
...
std::string testing::internal::String::FormatHexInt(int val) {
  std::ostringstream ss;
  ss << std::uppercase << std::hex << val;
  return ss.str();
}

std::string testing::internal::String::FormatHexUint32(uint32_t val) {
  std::ostringstream ss;
  ss << std::uppercase << std::hex << val;
  return ss.str();
}
...
// Minimal case-mapping tables (definitions moved into this translation unit to
// satisfy linker)
...
// NOTE: Unicode helper implementations removed from this .cpp to avoid
// multiple-definition (they are now provided as inline functions in
// ft_string.hpp)
