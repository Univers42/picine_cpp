/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   rpn_generator.cpp                                  :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: dlesieur <dlesieur@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/03/01 16:45:30 by dlesieur          #+#    #+#             */
/*   Updated: 2026/03/01 16:45:31 by dlesieur         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <iostream>
#include <vector>
#include <string>
#include <cstdlib>
#include <ctime>
#include <sstream>

// C++98 compliant number to string converter
template <typename T>
std::string to_string(T val) {
    std::stringstream ss;
    ss << val;
    return ss.str();
}

int main(int argc, char** argv) {
    int num_tests = 1000;
    if (argc > 1) num_tests = std::atoi(argv[1]);

    std::srand(std::time(NULL));
    const char ops[] = {'+', '-', '*', '/'};

    for (int i = 0; i < num_tests; ++i) {
        std::vector<long long> stack;
        std::string expr = "";

        // Randomly target an expression length of 3 to 22 tokens
        int target_len = (std::rand() % 20) + 3;
        int numbers_added = 0;
        int ops_added = 0;

        while (numbers_added + ops_added < target_len || stack.size() > 1) {
            // We can only add an operator if we have at least 2 numbers in the stack
            bool add_num = (stack.size() < 2) || (std::rand() % 2 == 0);

            // Force adding operators if we reached our target length to collapse the stack
            if (numbers_added + ops_added >= target_len && stack.size() > 1) {
                add_num = false;
            }

            if (add_num) {
                long long n = std::rand() % 10; // Numbers must be < 10
                stack.push_back(n);
                if (!expr.empty()) expr += " ";
                expr += to_string(n);
                numbers_added++;
            } else {
                char op = ops[std::rand() % 4];
                long long right = stack.back(); stack.pop_back();
                long long left = stack.back(); stack.pop_back();

                // Prevent Division by Zero
                if (op == '/' && right == 0) {
                    op = '+';
                }

                if (op == '+') stack.push_back(left + right);
                else if (op == '-') stack.push_back(left - right);
                else if (op == '*') stack.push_back(left * right);
                else if (op == '/') stack.push_back(left / right);

                if (!expr.empty()) expr += " ";
                expr += op;
                ops_added++;
            }
        }
        // Output format: Expression | Expected Result
        std::cout << expr << "|" << stack.back() << "\n";
    }
    return 0;
}
